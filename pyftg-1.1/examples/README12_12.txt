...
JSONPASSが変更がある場合は9行目で変更

FireBaseの参照ファイルは69行目
...
12/12追加
関数のみだとFirebaseの接続に干渉できないためclass化させました。
宣言の際は引数にplayerとオプションでuserである。playerはプレイヤー情報であり、TrueがP1でFalseがP2である。userに文字列を入れることで被りそうなときは識別可能、かぶっていてもあたらしいidでインスタンスが作られる。
reconnect()とdisconnect()で接続が可能レベルをaskLevel(wait,time)で変えれるようになったのでplayer入れないように注意